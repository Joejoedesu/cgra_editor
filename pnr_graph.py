...
class net_n:
    def __init__(self, net_id, width = 0, node_s = [], node_e = [], color = (0,0,0,0)):
        self.net_id = net_id
        self.width = width
        self.node_s = node_s
        self.node_e = node_e
    ...
    for net_id, net in routes.items():
        ...
        for route in net:
            ...
            for seg1, seg2 in zip(route, route[1:]):
                ...
                if seg1 == route[0]:
                    nets[net_id].node_s.append(node1)
                    nets[net_id].width = node1.bit_width

                node2 = graph.segment_to_node(seg2, net_id)
                graph.add_node(node2)

                if seg2 == route[-1]:
                    nets[net_id].node_e.append(node2)
